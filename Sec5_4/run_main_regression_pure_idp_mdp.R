...
input_csv <- "./data/paper2_main_pure_idp_mdp_input.parquet"
  ...
  for (oc in outcomes) {
    for (ls in lag_specs) {
      ...
      sample_rows[[model_id]] <- data.frame(
        ...
        treated_idp_rows = sum(d$ID015F == 1, na.rm = TRUE),
        control_mdp_rows = sum(d$ID015F == 0, na.rm = TRUE),
...
write.csv(coef_out, file.path(out_dir, "main_regression_pure_idp_mdp_coefficients.csv"), row.names = FALSE)
write.csv(core_terms, file.path(out_dir, "main_regression_pure_idp_mdp_core_terms.csv"), row.names = FALSE)
write.csv(stats_out, file.path(out_dir, "main_regression_pure_idp_mdp_model_stats.csv"), row.names = FALSE)
write.csv(samples_out, file.path(out_dir, "main_regression_pure_idp_mdp_samples.csv"), row.names = FALSE)

sink(file.path(out_dir, "main_regression_pure_idp_mdp_summaries.txt"))
cat("Paper2 main regressions, pure IDP vs pure MDP sample\n")
cat("Input:", input_csv, "\n")
cat("Sample rule: keep work_id with only IDP projects or only MDP projects; exclude mixed IDP/MDP work_id.\n")
cat("RS variable: RS\n")
