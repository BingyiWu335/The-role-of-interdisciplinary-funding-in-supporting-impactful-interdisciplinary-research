# Analysis code for TableA4 of Wu et al. (2026).
...
dir.create("./TableA4", recursive = TRUE, showWarnings = FALSE)
...
nsf_rs_hit  <- feglm(Hit15 ~ RSC*ID010F + ln_reference_count + ln_team_size + ln_institution_count + 
                       ln_A_HI_mean + ln_J_HI | year + field_id,
                     family = binomial(link = "logit"), data = df1, vcov = 'HC3')

nsfc_rs_hit  <- feglm(Hit15 ~ RSC*ID010F + ln_reference_count + ln_team_size + ln_institution_count +
                        ln_A_HI_mean + ln_J_HI | year + field_id,
                      family = binomial(link = "logit"), data = df2, vcov = 'HC3')

nih_rs_hit  <- feglm(Hit15 ~ RSC*ID010F + ln_reference_count + ln_team_size + ln_institution_count +
                       ln_A_HI_mean + ln_J_HI | year + field_id,
                     family = binomial(link = "logit"), data = df3, vcov = 'HC3')

all_rs_hit  <- feglm(Hit15 ~ RSC*ID010F + ln_reference_count + ln_team_size + ln_institution_count +
...
flextable::save_as_docx(tab, path = "./TableA4/Hit_RS_ID010F.docx")




nsf_rs_hit  <- feols(ln_NC1 ~ RSC*ID010F + ln_reference_count + ln_team_size + ln_institution_count +
                       ln_A_HI_mean + ln_J_HI | year + field_id, data = df1, vcov = "HC3")

nsfc_rs_hit  <- feols(ln_NC1 ~ RSC*ID010F + ln_reference_count + ln_team_size + ln_institution_count +
                        ln_A_HI_mean + ln_J_HI | year + field_id, data = df2, vcov = "HC3")

nih_rs_hit  <- feols(ln_NC1 ~ RSC*ID010F + ln_reference_count + ln_team_size + ln_institution_count +
                       ln_A_HI_mean + ln_J_HI | year + field_id, data = df3, vcov = "HC3")

all_rs_hit  <- feols(ln_NC1 ~ RSC*ID010F + ln_reference_count + ln_team_size + ln_institution_count +
                       ln_A_HI_mean + ln_J_HI | year + field_id + Funder, data = df4, vcov = "HC3")

tab <- msummary(list("nsf_rs_hit" = nsf_rs_hit, "nsfc_rs_hit" = nsfc_rs_hit,
                     "nih_rs_hit" = nih_rs_hit, "all_rs_hit" = all_rs_hit),
                fmt = 3, stars = TRUE, gof_omit = "AIC|FE|Within|RMSE|^R2$", output = "flextable") %>% autofit() %>% fontsize(size = 7)
flextable::save_as_docx(tab, path = "./TableA4/NC_RS_ID010F.docx")
...
nsf_rs_hit  <- feglm(Hit15 ~ RSC*ID020F + ln_reference_count + ln_team_size + ln_institution_count + 
                       ln_A_HI_mean + ln_J_HI | year + field_id,
                     family = binomial(link = "logit"), data = df1, vcov = 'HC3')

nsfc_rs_hit  <- feglm(Hit15 ~ RSC*ID020F + ln_reference_count + ln_team_size + ln_institution_count +
                        ln_A_HI_mean + ln_J_HI | year + field_id,
                      family = binomial(link = "logit"), data = df2, vcov = 'HC3')

nih_rs_hit  <- feglm(Hit15 ~ RSC*ID020F + ln_reference_count + ln_team_size + ln_institution_count +
                       ln_A_HI_mean + ln_J_HI | year + field_id,
                     family = binomial(link = "logit"), data = df3, vcov = 'HC3')

all_rs_hit  <- feglm(Hit15 ~ RSC*ID020F + ln_reference_count + ln_team_size + ln_institution_count +
                       ln_A_HI_mean + ln_J_HI | year + field_id + Funder,
                     family = binomial(link = "logit"), data = df4, vcov = 'HC3')
...
tab <- msummary(list("nsf_rs_hit" = nsf_rs_hit, "nsfc_rs_hit" = nsfc_rs_hit,
                     "nih_rs_hit" = nih_rs_hit, "all_rs_hit" = all_rs_hit),
                fmt = 3, stars = TRUE, gof_omit = "AIC|FE|Within|RMSE|^R2$", output = "flextable") %>% autofit() %>% fontsize(size = 7)
flextable::save_as_docx(tab, path = "./TableA4/Hit_RS_ID020F.docx")




nsf_rs_hit  <- feols(ln_NC1 ~ RSC*ID020F + ln_reference_count + ln_team_size + ln_institution_count +
                       ln_A_HI_mean + ln_J_HI | year + field_id, data = df1, vcov = "HC3")

nsfc_rs_hit  <- feols(ln_NC1 ~ RSC*ID020F + ln_reference_count + ln_team_size + ln_institution_count +
                        ln_A_HI_mean + ln_J_HI | year + field_id, data = df2, vcov = "HC3")

nih_rs_hit  <- feols(ln_NC1 ~ RSC*ID020F + ln_reference_count + ln_team_size + ln_institution_count +
                       ln_A_HI_mean + ln_J_HI | year + field_id, data = df3, vcov = "HC3")

all_rs_hit  <- feols(ln_NC1 ~ RSC*ID020F + ln_reference_count + ln_team_size + ln_institution_count +
                       ln_A_HI_mean + ln_J_HI | year + field_id + Funder, data = df4, vcov = "HC3")

tab <- msummary(list("nsf_rs_hit" = nsf_rs_hit, "nsfc_rs_hit" = nsfc_rs_hit,
                     "nih_rs_hit" = nih_rs_hit, "all_rs_hit" = all_rs_hit),
                fmt = 3, stars = TRUE, gof_omit = "AIC|FE|Within|RMSE|^R2$", output = "flextable") %>% autofit() %>% fontsize(size = 7)
flextable::save_as_docx(tab, path = "./TableA4/NC_RS_ID020F.docx")
